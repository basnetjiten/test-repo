...
class WebStrategy(PlatformStrategy):
    """Execution strategy handling linting, dependencies, and tests for web client frameworks (Next.js)."""
    ...
    async def prepare(self, repo_path: Path, _branch_name: str) -> None:
        """
        Resolve Web project dependencies.
        """
    ...
    async def validate(self, repo_path: Path) -> list[str]:
        ...
        """
        logger.info("Validating Web repository at %s", repo_path)
        package_json = repo_path / "package.json"
        errors: list[str] = []
        ...
        if await AsyncFileSystemService.exists(package_json):
            ...
            if changed_files:
                logger.info("Running Next.js linting on %d modified files...", len(changed_files))
            ...
            try:
                ...
                if "test" in scripts:
                    ...
                    if returncode != 0:
                        errors.append(f"Web tests failed: {err.decode().strip()}")
    ...
    async def bootstrap(self, _repo_path: Path, starter_type: str) -> None:
        """
        Bootstrap Web client boilerplate.
        """
