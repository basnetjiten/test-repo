    ...
    async def publish_single_platform(platform: str) -> str | None:
        ...
        if git.is_git_repo():
            ...
            if not git.has_changes():
                ...
                commit_msg = f"feat: [{ctx.jira_ticket.id}] {ctx.feature_name} ({platform})"
...
def _create_github_pr(ctx, branch_name: str) -> str | None:
    ...
    payload = {
        "title": f"feat: [{ctx.jira_ticket.id}] {ctx.feature_name}",
        "head": branch_name,
