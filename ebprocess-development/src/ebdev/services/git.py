...
class GitService:
    ...
    def clone_or_fetch(self, target_repo: str, starter_kit_url: str | None = None) -> list[str]:
        """
        ...
        if not self.is_git_repo():
            if starter_kit_url and target_repo:
                ...
                try:
                    ...
                    subprocess.run(
                        ["git", "clone", starter_clone_url, str(self.repo_path)],
                        check=True,
                        env=self.env,
                        capture_output=True,
                        text=True,
    ...
    def sync_with_main(self, default_branch: str = "main") -> list[str]:
        ...
        fetch_res = self._run(["fetch", "origin", f"{default_branch}:{default_branch}"], check=False)
        if fetch_res.returncode != 0:
            log.append(f"Could not fetch origin/{default_branch} (it may not exist yet). Skipping sync.")
            return log

        merge_res = self._run(["merge", f"origin/{default_branch}"], check=False)
        if merge_res.returncode != 0:
            self._run(["merge", "--abort"], check=False)
            error_msg = f"Merge conflict detected when syncing with origin/{default_branch}."
                ...
                try:
                    ...
                    if res.status_code == 404:
                        ...
                        if create_res.status_code in (200, 201):
                            ...
                            return True
                        logger.error(
                            "Failed to create Bitbucket repository: %s - %s", create_res.status_code, create_res.text
                        )
                        return False
                ...
                try:
                    ...
                    if res.status_code == 404:
                        ...
                        if create_res.status_code in (200, 201):
                            ...
                            return True
                        logger.error(
                            "Failed to create GitHub repository: %s - %s", create_res.status_code, create_res.text
                        )
                        return False
